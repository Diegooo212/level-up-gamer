...
function renderUser() {
    const userInfo = document.getElementById("userActions");
    const menu = document.getElementById("mainMenu");
    const user = JSON.parse(localStorage.getItem("user"));
    if (!userInfo || !menu) return;

    if (user) {
        userInfo.innerHTML = `<span>👤 ${user.email}</span><button onclick="logout()">Cerrar Sesión</button>`;
...
function renderCarrito() {
    // ... (Tu función renderCarrito que ya funciona bien) ...
    ...
    carrito.forEach((item, i) => {
        ...
        carritoBody.innerHTML += `
            <tr>
                ...
                <td><button onclick="changeQuantity(${i}, -1)">➖</button> ${item.cantidad} <button onclick="changeQuantity(${i}, 1)">➕</button></td>
...
function procesarCompra() {
    // ... (Tu función procesarCompra que ya funciona bien) ...
    const user = JSON.parse(localStorage.getItem("user"));
    if (!user) { alert("Debes iniciar sesión para finalizar la compra."); return; }
    const carrito = JSON.parse(localStorage.getItem("carrito")) || [];
    ...
    let descuento = esAlumnoDuoc ? subtotal * 0.15 : 0;
    const totalFinal = subtotal - descuento;
    ...
    localStorage.removeItem("carrito");
    alert("¡Compra realizada con éxito! Gracias por elegir Level-Up Gamer.");
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // =======================================================================
    // --- NUEVA LÓGICA PARA PÁGINAS DE PRODUCTOS (CARGA DINÁMICA Y FILTROS) ---
    // =======================================================================
    const productGrid = document.querySelector(".productos-grid");
    if (productGrid) {
        // 1. CARGA DINÁMICA DE PRODUCTOS
        const allProducts = JSON.parse(localStorage.getItem("products")) || [];
        const pageTitle = document.querySelector("h2").textContent.toLowerCase();
        let categoryToShow;

        // Asigna la categoría basado en el título de la página
        if (pageTitle.includes("consolas")) categoryToShow = "consola";
        else if (pageTitle.includes("accesorios")) categoryToShow = "accesorio";
        else if (pageTitle.includes("sillas gamer")) categoryToShow = "silla gamer";
        else if (pageTitle.includes("computadores")) categoryToShow = "computador";

        const productsToShow = allProducts.filter(p => p.category === categoryToShow);
        
        productGrid.innerHTML = ""; // Limpiamos la grilla por si había algo
        if (productsToShow.length === 0) {
            ...
            productsToShow.forEach(product => {
                ...
                if (product.onSale && product.discountPrice > 0) {
                    priceHTML = `
                        <p class="precio-oferta"><b>Oferta:</b> $${parseFloat(finalPrice).toLocaleString('es-CL')}</p>
                        <p class="precio-original-tachado">Normal: $${parseFloat(product.price).toLocaleString('es-CL')}</p>
                    `;
                } else {
                    priceHTML = `<p><b>Precio:</b> $${parseFloat(product.price).toLocaleString('es-CL')}</p>`;
                }
                
                const productForCart = { ...product, price: finalPrice };

                const cardHTML = `
                    <div class="card" data-precio="${finalPrice}" data-marca="${product.brand.toLowerCase()}">
                        <img src="${product.img}" alt="${product.name}">
                        <h3>${product.name}</h3>
                        <p><b>Marca:</b> ${product.brand}</p>
                        ${priceHTML}
                        <p><b>Stock:</b> ${product.stock} unidades</p>
                        <button class="buy-btn" onclick='addToCart(${JSON.stringify(productForCart)})'>🛒 Comprar</button>
                    </div>
                `;
        ...
        // 2. INICIALIZACIÓN DE FILTROS (ahora que las tarjetas ya existen en la página)
